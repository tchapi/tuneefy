var _COOKIE = "tuneefyPrefs";
var _COOKIE_2 = "tuneefyHelpBox";
var _COOKIE_3 = "tuneefySearchType";

function SearchUI(initialController){
...
SearchUI.prototype.initUIInitialState = function() {
  ...
  if(typeof $.iphoneStyle == 'function') {
    ...
    this.strictModeCheckBox.iphoneStyle({
      checkedLabel: _yes, uncheckedLabel: _no,
  ...
  var cookieValue = document.cookie.split(_COOKIE + '=')[1];
  
  if (cookieValue == null)
    cookieValue = _default_platforms;
  else
  ...
  cookieValue = document.cookie.split(_COOKIE_3 + '=')[1];

  if (cookieValue != null) {
    if (cookieValue.split(";")[0] == 'albums') this.searchTypeCheckBox.click();
  }
...
SearchUI.prototype.bindObjects = function() {
  ...
  this.form.submit($.proxy(function(e){
    ...
    var strictMode = this.strictModeCheckBox.is(':checked');
    ...
    if (queryString == "" || queryString == _query_label || this.selectedPlatforms == "") return false;
  ...
  this.queryField.click($.proxy(function(e){
    ...
    if (this.queryField.val() == _query_label) {
      this.queryField.val("");
    }
    ...
    var cookieValue = document.cookie.split(_COOKIE_2 + '=')[1];
  ...
  this.queryField.keyup($.proxy(function(e) {
    if (this.queryField.val() != _query_label && $.trim(this.queryField.val()).length != 0)
  ...
  this.closeForever.click($.proxy(function(e){
    ...
    // Sets the cookie_2
    var value = _COOKIE_2 + "=neverAgain; ";
    value += "expires=Sat, 01 Feb 2042 01:20:42 GMT; path=/; domain= .tuneefy.com;"; 
    document.cookie= value;
    $("#help").fadeOut();
    this.queryField.focus();
  ...
  $('html').click($.proxy(function() {
    ...
    if (this.queryField.val() == "") {
      this.queryField.val(_query_label);
    }
    
  }, this));

  // In case we click inside the options div, we must not close it
  this.advanced.click(function(e){ e.stopPropagation(); });
  ...
  this.platforms.click($.proxy(function(e){
    ...
    var tempSelectedPlatforms = new Array();
    ...
    // Sets the cookie
    var value = _COOKIE + "=" + encodeURIComponent(this.selectedPlatforms) + "; ";
    value += "expires=Sat, 01 Feb 2042 01:20:42 GMT; path=/; domain= .tuneefy.com;"; 
  ...
  /******* SEARCH TRIGGERED *******/
  $(this.controller).bind("tuneefy.search.trigger", $.proxy(function(e){
  ...
  $(this.controller).bind("tuneefy.search.newResultsReady", $.proxy(function(e, query, pltf){
