...
SliderUI.prototype.goNext = function() {
  ...
  if(next.length == 0) { next = this.picks.first();}
  
  next.css("left","400px");
  next.show();
  next.animate({"left": "30px"}, "slow");
  ...
  active.animate({"left": "-300px"}, "slow");
  active.fadeOut("slow");
...
SliderUI.prototype.goPrev = function() {
  ...
  if(prev.length == 0) { prev = this.picks.last();}
  
  prev.css("left","-300px");
  prev.show();
  prev.animate({"left": "30px"}, "slow");
  ...
  active.animate({"left": "400px"}, "slow");
  active.fadeOut("slow");
...
SliderUI.prototype.initUIInitialState = function() {
  ...
  /* TRICK */
  var myself = this;
  function callMethod() { myself.goNext(); }
