...
// For reference : 
// Platform::__construct($key, $secret, $default, $activeSearch, $activeAlbumSearch, $activeLookup, $order)
...
API::addPlatform(new GROOVESHARK(null, null, false, false, false, false, _GROOVESHARK), 97);
/* ********** */

/* DEPRECATED */
define('_JIWA', 4);
API::addPlatform(new JIWA(null, null, false, false, false, false, _JIWA), 98);
/* ********** */

define('_SOUNDCLOUD', 5);
API::addPlatform(
    new SOUNDCLOUD(
        $options['SOUNDCLOUD']['key'],
...
API::addPlatform(
    ...
    $options['ITUNES']['order']
); // no lookup for iTunes

define('_ECHONEST', 12);
API::addPlatform(
    new ECHONEST(
        $options['ECHONEST']['key'],
...
function web($url)
{
    ...
    } elseif (substr($url, 0, 13) == "spotify:album") {
