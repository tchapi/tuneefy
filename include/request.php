...
if (isset($_GET['r']) && $continue != false) {
    if ($_GET['r'] == 'trends') {
        if (!_DESACTIVATE_TRENDS && $mobile === false) {
            $action = 'trends';
        } else {
            $action ='search';
        }
        ...
        if (!_DESACTIVATE_PLAYLISTS && $mobile === false) {
            $action = 'playlists';
        } else {
            $action ='search';
        }
...
if (isset($_GET['q']) && isset($_GET['widget']) && $continue != false) {
    ...
    $action = 'widget';
} elseif (isset($_GET['q']) && $continue != false) {
    $request = trim(stripslashes(urldecode($_GET['q'])));
    $action = 'search';
} elseif (isset($_GET['t']) && $continue != false) {
    $request = trim(stripslashes($_GET['t']));
    $action = 'track';
} elseif (isset($_GET['a']) && $continue != false) {
